...
export async function completeSampleFollowupAction(input: {
  followupId: string;
  contact_type?: VisitContactType;
  conversation_summary?: string;
  contact_person_name?: string;
  ...
  try {
    ...
    const summary =
      input.conversation_summary?.trim() ||
      `Follow-up concluído: ${followup.title ?? "amostra"}`;
    ...
    revalidatePath("/");
    revalidatePath("/visitas");
    revalidatePath(`/amostras/${followup.related_id}`);

    return { success: "Follow-up da amostra registrado." };
  } catch (error) {
    return {
      error:
        error instanceof Error
